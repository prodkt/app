...
} from '@prodkt/helpers/date-time.helper'
...
export const generateCalendarData = (
  ...
  for (let week = 0; week < numWeeks; week++) {
    ...
    const weekNumber = getWeekNumberOfDate(
      new Date(year, month, week * 7 - firstDayOfMonth + 1),
    )
    ...
    for (let i = 0; i < 7; i++) {
      ...
      if (formattedDatePayload)
        currentWeekObject[formattedDatePayload] = {
          ...
          is_current_week:
            getWeekNumberOfDate(date) === getWeekNumberOfDate(new Date()),
          is_today: date.toDateString() === new Date().toDateString(),
        }
    }

    calendarData[`y-${year}`][`m-${month}`][`w-${weekNumber}`] =
      currentWeekObject
  }

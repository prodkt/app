...
// constants
import { ISSUE_DISPLAY_FILTERS_BY_LAYOUT } from '@prodkt/constants/issue'
import { STATE_GROUPS } from '@prodkt/constants/state'
// helpers
import { orderArrayBy } from '@prodkt/helpers/array.helper'
...
export const handleIssuesMutation: THandleIssuesMutation = (
  ...
  if (Array.isArray(prevData)) {
    ...
    if (selectedGroupBy === 'priority')
      ...
      newGroup = prevData[formData.state_id ?? ''] ?? []
...
export const shouldHighlightIssueDueDate = (
  ...
  if (
    [STATE_GROUPS.completed.key, STATE_GROUPS.cancelled.key].includes(
      stateGroup,
    )
  )
    return false
...
export const getDescriptionPlaceholder = (
  ...
  const isDescriptionEmpty =
    !description || description === '<p></p>' || description.trim() === ''
...
export const issueCountBasedOnFilters = (
  ...
  if (['spreadsheet', 'gantt_chart'].includes(layout)) {
    ...
    if (groupBy && subGroupBy) {
      Object.keys(issueIds || {}).map((groupId) => {
        Object.keys((issueIds as TSubGroupedIssues)?.[groupId] || {}).map(
          (subGroupId) => {
            issuesCount +=
              (issueIds as TSubGroupedIssues)?.[groupId]?.[subGroupId]
                ?.length || 0
          },
        )
      })

...
import {
  ...
  EDurationFilters,
} from '@prodkt/constants/dashboard'
// types
import { TIssuesListTypes } from '@prodkt/types'
...
export const getDurationFilterDropdownLabel = (
  ...
  if (duration !== 'custom')
    return (
      DURATION_FILTER_OPTIONS.find((option) => option.key === duration)
        ?.label ?? ''
    )
  else {
    const afterDate = customDates
      .find((date) => date.includes('after'))
      ?.split(';')[0]
    const beforeDate = customDates
      .find((date) => date.includes('before'))
      ?.split(';')[0]

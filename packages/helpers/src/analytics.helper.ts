...
// helpers
import { DATE_KEYS } from '@prodkt/constants/analytics'
import { MONTHS_LIST } from '@prodkt/constants/calendar'
import { STATE_GROUPS } from '@prodkt/constants/state'
import {
  addSpaceIfCamelCase,
  capitalizeFirstLetter,
  generateRandomColor,
} from '@prodkt/helpers/string.helper'
import {
  IAnalyticsData,
  IAnalyticsParams,
  IAnalyticsResponse,
  ...
  Object.keys(response).forEach((key) => {
    ...
    if (params.segment) {
      response[key].map((item: any) => {
        ...
        if (!xAxisKeys.includes(item.segment ?? 'None'))
          xAxisKeys.push(item.segment ?? 'None')
      })
...
export const generateBarColor = (
  ...
  if (params[type] === 'state_id')
    color = analytics?.extras.state_details.find(
      (s) => s.state_id === value,
    )?.state__color

  if (params[type] === 'labels__id')
    color =
      analytics?.extras.label_details.find((l) => l.labels__id === value)
        ?.labels__color ?? undefined

  if (params[type] === 'state__group')
    color =
      STATE_GROUPS[value.toLowerCase() as TStateGroups]?.color ?? undefined
...
export const generateDisplayName = (
  ...
  if (params[type] === 'assignees__id')
    displayName =
      analytics?.extras.assignee_details.find((a) => a.assignees__id === value)
        ?.assignees__display_name ?? 'No assignee'

  if (params[type] === 'issue_cycle__cycle_id')
    displayName =
      analytics?.extras.cycle_details.find(
        (c) => c.issue_cycle__cycle_id === value,
      )?.issue_cycle__cycle__name ?? 'None'

  if (params[type] === 'issue_module__module_id')
    displayName =
      analytics?.extras.module_details.find(
        (m) => m.issue_module__module_id === value,
  ...
  if (params[type] === 'state_id')
    displayName =
      analytics?.extras.state_details.find((s) => s.state_id === value)
        ?.state__name ?? 'None'

  if (DATE_KEYS.includes(params.segment ?? ''))
    displayName = renderMonthAndYear(value)

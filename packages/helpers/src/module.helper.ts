...
export const orderModules = (
  ...
  if (orderByKey === 'name')
    orderedModules = sortBy(modules, [(m) => m.name.toLowerCase()])
  if (orderByKey === '-name')
    orderedModules = sortBy(modules, [(m) => m.name.toLowerCase()]).reverse()
  if (['progress', '-progress'].includes(orderByKey))
    orderedModules = sortBy(modules, [
      (m) => {
        let progress =
          (m.completed_issues + m.cancelled_issues) / m.total_issues
  ...
  if (['issues_length', '-issues_length'].includes(orderByKey))
    orderedModules = sortBy(modules, [
      (m) =>
        orderByKey === 'issues_length' ? m.total_issues : !m.total_issues,
      'name',
    ])
  if (orderByKey === 'target_date')
    orderedModules = sortBy(modules, [(m) => m.target_date])
  if (orderByKey === '-target_date')
    orderedModules = sortBy(modules, [(m) => !m.target_date])
  if (orderByKey === 'created_at')
    orderedModules = sortBy(modules, [(m) => m.created_at])
  if (orderByKey === '-created_at')
    orderedModules = sortBy(modules, [(m) => !m.created_at])
...
export const shouldFilterModule = (
  ...
  Object.keys(filters).forEach((key) => {
    ...
    if (filterKey === 'status' && filters.status && filters.status.length > 0)
      fallsInFilters =
        fallsInFilters &&
        filters.status.includes(module.status?.toLowerCase() ?? '')
    if (filterKey === 'lead' && filters.lead && filters.lead.length > 0)
      fallsInFilters =
        fallsInFilters && filters.lead.includes(`${module.lead_id}`)
    if (
      filterKey === 'members' &&
      filters.members &&
    ...
    if (
      ...
      filters.start_date.forEach((dateFilter) => {
        fallsInFilters =
          fallsInFilters &&
          !!startDate &&
          satisfiesDateFilter(startDate, dateFilter)
      })
    }
    if (
      filterKey === 'target_date' &&
      filters.target_date &&
      filters.target_date.length > 0
    ) {
      ...
      filters.target_date.forEach((dateFilter) => {
        fallsInFilters =
          fallsInFilters &&
          !!endDate &&
          satisfiesDateFilter(endDate, dateFilter)
      })
    }
  })

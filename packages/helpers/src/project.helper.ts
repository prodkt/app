...
// constants
import { EUserProjectRoles } from '@prodkt/constants/project'
// helpers
import { getDate } from '@prodkt/helpers/date-time.helper'
import { satisfiesDateFilter } from '@prodkt/helpers/filter.helper'
// types
import {
  IProject,
...
export const orderJoinedProjects = (
  ...
  if (destinationIndex === 0) {
    ...
    const currentSortOrder =
      joinedProjects[destinationIndex - 1].sort_order || 0
    updatedSortOrder = currentSortOrder + sortOrderDefaultValue
  } else {
    ...
    const destinationBottomProjectSortOrder =
      joinedProjects[destinationIndex].sort_order || 0
    const updatedValue =
      (destinationTopProjectSortOrder + destinationBottomProjectSortOrder) / 2
...
export const shouldFilterProject = (
  ...
  Object.keys(filters).forEach((key) => {
    ...
    if (filterKey === 'access' && filters.access && filters.access.length > 0)
      fallsInFilters =
        fallsInFilters && filters.access.includes(`${project.network}`)
    if (filterKey === 'lead' && filters.lead && filters.lead.length > 0)
      fallsInFilters =
        fallsInFilters && filters.lead.includes(`${project.project_lead}`)
    if (
      filterKey === 'members' &&
      filters.members &&
      ...
      fallsInFilters =
        fallsInFilters &&
        filters.members.some((memberId) => memberIds.includes(memberId))
    }
    if (
      ...
      filters.created_at.forEach((dateFilter) => {
        fallsInFilters =
          fallsInFilters &&
          !!createdDate &&
          satisfiesDateFilter(createdDate, dateFilter)
      })
    }
  })
  if (displayFilters.my_projects && !project.is_member) fallsInFilters = false
  if (displayFilters.archived_projects && !project.archived_at)
...
export const orderProjects = (
  ...
  if (orderByKey === 'sort_order')
    orderedProjects = sortBy(projects, [(p) => p.sort_order])
  if (orderByKey === 'name')
    orderedProjects = sortBy(projects, [(p) => p.name.toLowerCase()])
  if (orderByKey === '-name')
    orderedProjects = sortBy(projects, [(p) => p.name.toLowerCase()]).reverse()
  if (orderByKey === 'created_at')
    orderedProjects = sortBy(projects, [(p) => p.created_at])
  if (orderByKey === '-created_at')
    orderedProjects = sortBy(projects, [(p) => !p.created_at])

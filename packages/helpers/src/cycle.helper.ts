...
import { ICycle, TCycleFilters } from '@prodkt/types'
 ...
 * @returns {boolean}
 */
export const shouldFilterCycle = (
  cycle: ICycle,
  filter: TCycleFilters,
): boolean => {
  ...
  Object.keys(filter).forEach((key) => {
    ...
    if (filterKey === 'status' && filter.status && filter.status.length > 0)
      fallsInFilters =
        fallsInFilters &&
        filter.status.includes(cycle.status?.toLowerCase() ?? '')
    if (
      filterKey === 'start_date' &&
      filter.start_date &&
      filter.start_date.length > 0
    ) {

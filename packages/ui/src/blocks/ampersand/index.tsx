...
const Ampersand = React.forwardRef<HTMLDivElement>(({ ...props }, ref) => {
  return (
    <section
      ref={ref}
      ...
      {...props}
    >
      <div className='absolute bottom-0 top-auto z-10 mb-0 mt-auto h-[160px] w-full bg-gradient-to-b from-transparent to-[var(--violet1)]'></div>
      <div className='absolute inset-0 size-full bg-[var(--mauve1)]'></div>
      <img
        src={Background}
        decoding='async'
        loading='lazy'
        alt='prodkt'
        className='absolute inset-x-auto inset-y-0 z-0 mx-0 my-auto h-full w-dvw select-none object-cover object-center opacity-[5%] contrast-200 saturate-0'
      />
      <div className='container relative left-0 mx-auto pt-[92px] md:py-[6.5dvw]'>
        <div className='inset-x-0 mx-auto flex w-full max-w-6xl flex-row items-start justify-center gap-0 text-center sm:align-top md:items-center md:align-middle'>
          <div className='left-0 top-0 z-20 me-auto ms-auto flex w-1/2 scale-100 flex-col place-content-start items-start justify-start gap-[3dvw] text-start align-middle sm:w-3/4 sm:scale-90 sm:justify-center md:w-1/2 md:grow md:scale-100'>
            ...
            <h2 className='xs:text-5xl z-10 mx-auto me-auto ms-0 flex max-w-3xl flex-col gap-[1dvw] pb-2 text-start text-3xl font-medium tracking-tighter xl:text-5xl'>
              <span className='me-auto ms-0 block w-full max-w-md bg-gradient-to-l from-[var(--mauve9)] to-[var(--slate10)] bg-clip-text text-3xl tracking-tighter text-transparent'>
                <span className='text-[var(--violet12)]'>Figma</span>
                <span className='inline-flex h-8 -translate-y-1 items-center justify-center rounded-lg bg-[var(--violet9)] px-2 pb-1 pt-0 align-middle text-[var(--violet1)]'>
                  to
                </span>{' '}
          ...
          <div className='group relative top-0 flex w-1/2 max-w-4xl scale-100 flex-row place-content-center items-center justify-center align-middle sm:w-1/4 md:w-auto md:shrink md:scale-100'>
            <img
              src={AmpersandMock}
              decoding='async'
              loading='lazy'
            ...
            <img
              ...
              loading='lazy'
              alt='prodkt'
              className='invisible absolute inset-y-auto left-0 right-auto z-[1] my-auto me-auto ms-0 w-[300px] origin-center -translate-x-[120px] select-none contrast-125 saturate-0 md:visible'
            />
            <img
              src={CenterCircle}
              decoding='async'
              loading='lazy'
              alt='prodkt'
              className='group-hover:scale-25 absolute inset-x-auto z-[22] my-auto me-auto ms-auto min-w-[650px] origin-center scale-50 select-none brightness-125 contrast-125 transition-all duration-500 md:scale-100 md:group-hover:scale-75'
            />
            <img
              src={AppCircles}
              decoding='async'
            ...
            />
            <div className='absolute z-50 my-auto me-auto ms-auto h-[80px] w-[100px] animate-[spin_5s_linear_infinite] rounded-full bg-gradient-to-br from-[var(--mauve12)] to-[var(--mauve1)] mix-blend-hard-light blur-xl'></div>
          </div>
        </div>
      </div>
    </section>

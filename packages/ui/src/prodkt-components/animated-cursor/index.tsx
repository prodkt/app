...
export const AnimatedCursor: React.FC<{ className?: string; text: string }> = ({
  ...
  <motion.div
    ...
    className={'flex w-fit items-center gap-4 text-nowrap'}
  >
    <Cursor className={'-translate-y-5 translate-x-5 -scale-x-100'} />
    <div
      className={cn(
        'w-fit rounded-full border border-[var(--accentA5)] bg-[var(--accentA5)] px-2 py-0.5 text-xs font-normal',
        className,
      )}
    >
      {text}

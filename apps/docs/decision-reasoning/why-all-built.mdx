...
```bash
bun turbo dev --filter=@prodkt/web... --filter=!./packages/config/* --filter=!@prodkt/ui --only
```
...
{
  "scripts": {
    "dev:web:api": "bun turbo dev --filter=@prodkt/web... --filter=!./packages/config/* --filter=!@prodkt/ui --only"
  }
}
```
